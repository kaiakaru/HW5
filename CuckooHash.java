...
public class CuckooHash<K, V> {
    ...
    public void put(K key, V value) {
        ...
        while (cycleCount < CAPACITY) {
            // Check if the spot at `pos` is available or has a duplicate key with the same value
            if (table[index] == null || (table[index].getBucKey().equals(key) && table[index].getValue().equals(value))) {
                if (table[index] == null) {
                }
                table[index] = newBucket;
                return;
            }
            if (visitedPositions.contains(index)) {
                break;
            }
            visitedPositions.add(index);

            // Kick out the existing bucket at this position
            Bucket<K, V> displacedBucket = table[index];
            table[index] = newBucket;


            // Move displaced bucket to its other location
            index = (index == hash1(displacedBucket.getBucKey())) ? hash2(displacedBucket.getBucKey()) : hash1(displacedBucket.getBucKey());
            newBucket = displacedBucket;
            cycleCount++;
        }

        // If we've reached the cycle count, rehash
        rehash();
        put(key, value);  // Try to insert the element again post-rehash
